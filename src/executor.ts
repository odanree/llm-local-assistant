...
`;
    ...
    // Class-merging guidance: only mandate cn() when the workspace actually has a cn utility.
    // On greenfield workspaces with no .lla-profile.json (or profile says cnUtilityPath===null),
    // the LLM otherwise obediently imports `import { cn } from '@/utils/cn'` from the example
    // below, the file fails Cross-file Contract validation, and the corrector loops to death.
    const cnUtilityPath = this.config.projectProfile?.cnUtilityPath ?? null;
    const classMergingSection = cnUtilityPath
      ? `
...
`
      : `
CLASS MERGING RULE (mandatory): This workspace has NO cn() / clsx / classnames utility.
- NEVER import cn from any path — there is no cn.ts in this workspace
- NEVER import clsx or classnames — they are not installed
- Use plain string className props: className="px-4 py-2 text-sm"
- For conditional classes, use template literals or ternaries: className={\`px-4 \${active ? 'bg-blue-500' : 'bg-gray-200'}\`}
- If the component accepts a 'className' prop, append it with a template literal: className={\`px-4 py-2 \${className ?? ''}\`}
...
Do NOT include: backticks, markdown, explanations, other files, instructions`;
    ...
    // Cached on step.metadata across the outer retry loop. Without this cache the
    // architect regenerates criteria on every retry — and because the LLM branch is
    // non-deterministic, consecutive retries can produce contradictory requirements
    // (e.g. "destructure useAuthStore()" on retry 1 and "no hook calls, no store
    // imports" on retry 2). No model can satisfy both. Freezing criteria once per
    // step guarantees the corrector chases a stable target.
    const cachedCriteria = step.metadata?.acceptanceCriteria as string[] | undefined;
    const acceptanceCriteria = cachedCriteria
      ?? await this.generateAcceptanceCriteria(step, sourceReadContents[0]?.content);
    if (!cachedCriteria && acceptanceCriteria.length > 0) {
      step.metadata = { ...(step.metadata ?? {}), acceptanceCriteria };
    }
  ...
  private async writeAndVerify(
    ...
    for (let tscPass = 0; tscPass < 2 && tscErrors.length > 0; tscPass++) {
      ...
      // Deterministic pre-pass: SmartAutoCorrection handles a growing list of model-output
      // training-data artifacts (Zustand v3 default import, phantom cn, split React imports,
      // bare-string classNames, etc.). Apply these first — they're free, deterministic, and
      // strictly more reliable than an LLM correction call for patterns we've named.
      const preFixed = SmartAutoCorrection.fixCommonPatterns(finalContent, tscErrors, step.path!);
      if (preFixed !== finalContent) {
        finalContent = preFixed;
        await vscode.workspace.fs.writeFile(filePath, Buffer.from(finalContent, 'utf8'));
        tscErrors = await this.runTscCheck(workspaceUri.fsPath, step.path!);
        if (tscErrors.length === 0) {
          this.config.onMessage?.(`✅ Deterministic pre-pass resolved all TypeScript errors`, 'info');
          break;
        }
      }
