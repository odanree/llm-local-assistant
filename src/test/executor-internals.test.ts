...
export const useUserStore = create<UserState>((set) => ({
  ...
  /**
   * ========================================================================
   * TIER 2: validateArchitectureRules() - Medium Complexity (CC ≈ 7)
   * ========================================================================
   * Tests rule enforcement for:
   * 1. TanStack Query usage (no direct fetch)
   * 2. Zustand adoption (no Redux)
   * 3. Functional components (no class components)
   * 4. TypeScript strict mode (return type annotations)
   * 5. Zod validation rules
   * 6. React Hook Form patterns
   */
  const validateArchitectureRulesMatrix = [
    {
      name: 'Rule: TanStack Query required',
      content: 'const data = await fetch("/api/users");',
      rules: ['TanStack Query'],
      expected: { hasError: true, ruleType: 'fetch' },
      desc: 'Should flag direct fetch when TanStack Query required',
    },
    {
      name: 'Rule: TanStack Query - proper usage',
      content: 'const { data } = useQuery({ queryKey: ["users"], queryFn: fetchUsers });',
      rules: ['TanStack Query'],
      expected: { hasError: false },
      desc: 'Should accept proper useQuery pattern',
    },
    {
      name: 'Rule: Zustand required',
      content: 'const state = useSelector(s => s.user);',
      rules: ['Zustand'],
      expected: { hasError: true, ruleType: 'redux' },
      desc: 'Should flag Redux when Zustand required',
    },
    {
      name: 'Rule: Zustand - proper usage',
      content: 'const { user } = useUserStore();',
      rules: ['Zustand'],
      expected: { hasError: false },
      desc: 'Should accept proper Zustand store usage',
    },
    {
      name: 'Rule: Functional components required',
      content: 'export class MyComponent extends React.Component {}',
      rules: ['functional components'],
      expected: { hasError: true, ruleType: 'class' },
      desc: 'Should flag class component when functional required',
    },
    {
      name: 'Rule: TypeScript strict - missing return types',
      content: 'const fn = (x: number) => x * 2; function fn2(x: string) { return x.length; }',
      rules: ['strict TypeScript'],
      expected: { hasError: true, ruleType: 'returnType' },
      desc: 'Should flag missing return type annotations',
    },
    {
      name: 'Rule: Hook Form + Zod resolver pattern',
      content: `const schema = z.object({ email: z.string() });
const form = useForm({ resolver: async (data) => validateData(data) });`,
      rules: [],
      expected: { hasError: true, ruleType: 'hookForm' },
      desc: 'Should flag async validation instead of zodResolver',
    },
    {
      name: 'Rule: Hook Form + Zod - correct resolver',
      content: `import { zodResolver } from '@hookform/resolvers/zod';
const schema = z.object({ email: z.string() });
const form = useForm({ resolver: zodResolver(schema) });`,
      rules: [],
      expected: { hasError: false },
      desc: 'Should accept proper zodResolver pattern',
    },
  ];

  it.each(validateArchitectureRulesMatrix)(
    'validateArchitectureRules: $desc',
    async ({ content, rules, expected, filePath = 'src/app.ts' }) => {
      ...
      // Inject rules into llm config mock
      (instance as any).config.llmClient = {
        config: { architectureRules: rules },
      };

      const result = await (instance as any).validateArchitectureRules(content, filePath);
