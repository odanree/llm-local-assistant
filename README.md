...
[![Version](https://img.shields.io/badge/version-2.18.3-blue.svg)](CHANGELOG.md)
...
**Recent Releases:**
- **v2.18.3** (Current) - Grounded `/explain` + Duration Display: `/explain` now inlines relative-import sources into the prompt so the model cites real code instead of guessing (eliminated a Kysely→Drizzle misidentification in testing). Adds grounding rules to suppress speculation, ranks imports by signal (types/db/hooks before sibling components), tightens prefill caps for ~25% speedup on import-heavy files. Long-running responses (≥`llm-assistant.durationThresholdMs`, default 60s) now end with `⏱️ Took 1m 23s`.
- **v2.17.0** - Quiet Activation: RAG scan, embeddings, and project-profile detection now defer until the chat panel is opened. Installing the extension no longer writes `.lla-embeddings.json` / `.lla-index.json` / `.lla-profile.json` or pings Ollama for users who haven't tried it yet.
...
**v2.18.3** — Local AI Code Orchestrator | 🧪 2,872 Tests Passing | 📦 79KB Install | 🔒 100% Private | Zero-Telemetry
